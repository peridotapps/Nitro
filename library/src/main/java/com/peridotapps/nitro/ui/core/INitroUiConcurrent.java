...
public interface INitroUiConcurrent {
    ...
    default void runOnNewThread(Runnable action) {
        ConcurrentHandler.getSharedInstance().runOnNewThread(action);
    }

    default void runOnNewThread(Runnable action, long delay) {
        ConcurrentHandler.getSharedInstance().runOnNewThread(action, delay);
    }

    default void runOnUiThread(Runnable action) {
        ConcurrentHandler.getSharedInstance().runOnUiThread(action);
    }

    default void runOnUiThread(Runnable action, long delay) {
        ConcurrentHandler.getSharedInstance().runOnUiThread(action, delay);
    }
